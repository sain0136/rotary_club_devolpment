<template>
  <div class="admin-container">
    ...
    <h1 class="table-title" v-else>
      No District Administrators to display!
    </h1>
  </div>
</template>
...
export default {
  ...
  methods: {
    async deleteAdmin(userId) {
      if (
        confirm(
          `Are you sure you want to delete admin ${userId}?`,
        )
      ) {
        try {
          await district_admin.delete(
            userId,
          )
        } catch (err) {
          alert(err)
    ...
    async goToEditDistrictAdminPage(
      userId,
    ) {
      this.$router.push(
        `${userId}/edit`,
